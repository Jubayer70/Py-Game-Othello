...
class Othello(Board):
    ...
    def initialize_board(self):
        ...
        coord2 = int(self.n / 2)
        initial_square = [(coord1, coord2), (coord1, coord1),
                          (coord2, coord1), (coord2, coord2)]

        for i in range(len(initial_square)):
            color = i % 2
            row = initial_square[i][0]
            col = initial_square[i][1]
            self.board[row][col] = color + 1
            self.draw_tile(initial_square[i], color)
    ...
    def play(self, x, y):
        ...
        if not self.has_legal_move() or sum(self.num_tiles) == self.n ** 2:
            ...
            print('Thanks for playing Othello <3')
            close = input('Close the game screen? Y/N\n')
            if close == 'Y':
                turtle.bye()
            elif close != 'N':
                print('Quitting in 3s ...')
    ...
    def report_result(self):
        ...
        if self.num_tiles[0] > self.num_tiles[1]:
            print('You win! You have %d tiles, but the computer only has %d!' % (self.num_tiles[0], self.num_tiles[1]))
        elif self.num_tiles[0] < self.num_tiles[1]:
            print(
                'You Lose.. The computer has %d tiles, but you only have %d :' % (self.num_tiles[1], self.num_tiles[0]))
        else:
            print("It is a tie ! There are %d of each" % self.num_tiles[0])

    def __str__(self):
        player_str = 'current player ' + str(self.current_player + 1) + '\n'
        num_tile_str = '# of block tiles --1: ' + str(self.num_tiles[0]) + '\n' + '# of white tiles --2: ' + \
                       str(self.num_tiles[1]) + '\n'
        board_str = Board.__str__(self)
        printable_str = player_str + num_tile_str + board_str
